...
test("runtime reporters turn skipped Playwright and Node tests into failures", () => {
  ...
  cleanReporter.onBegin({}, { allTests: () => Array.from({ length: 705 }) });
  ...
  assert.match(messages.join("\n"), /incomplete suite.*expected=705, received=5/iu);
  ...
  try {
    ...
    assert.match(emptyNodeRun.stderr, /incomplete suite.*expected=74, received=1/iu);
...
test("the quality policy keeps local browser execution deterministic", () => {
  ...
  const postPolicyMutation = runBrowserGateAfterInitialPolicyEdits({
    ...
    "scripts/playwright_contract.js": (source) =>
      source
        ...
        .replace(
          "EXPECTED_PLAYWRIGHT_TEST_COUNT = 705",
  ...
  const erasedTestInventory = runPolicyAgainstWorkflowEdits({
    "scripts/playwright_contract.js": (source) =>
      source.replace("EXPECTED_PLAYWRIGHT_TEST_COUNT = 705", "EXPECTED_PLAYWRIGHT_TEST_COUNT = 0")
  });
  assert.notEqual(erasedTestInventory.status, 0);
  assert.match(erasedTestInventory.stderr, /digest and 705-test contract.*independent policy literal/iu);
